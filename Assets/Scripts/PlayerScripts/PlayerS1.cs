...
public class PlayerS1 : PlayerMove_original
{
    ...
    protected override void Askill()
    {
        ...
        if (skillTime == 0){
            ...
            this.transform.position = new Vector3(transform.position.x + (2 * derect), this.transform.position.y, this.transform.position.z);
            skillTime = skillcool;
            useskill = true;
        }
        if (useskill && isswing)
